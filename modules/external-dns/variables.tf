###############################################################################
# Inputs for the external-dns AWS-side prerequisites. The in-cluster install
# (HelmRelease, domain filter, txt-owner-id) lives in gitops-flux; this module
...
variable "hosted_zone_id" {
  description = "Route 53 public hosted zone external-dns may manage records in. The ChangeResourceRecordSets statement is scoped to this zone alone."
  type        = string
}

variable "namespace" {
  description = "Namespace the external-dns controller runs in. Half of the IRSA trust condition's service-account subject."
  type        = string
...
variable "managed_record_types" {
  description = "Record types external-dns is allowed to change. A/AAAA/CNAME are the endpoint records it publishes; TXT is its ownership registry. Deliberately excludes NS, SOA and CAA so a compromised controller cannot delegate a subdomain away or authorise a different certificate authority."
  type        = list(string)
