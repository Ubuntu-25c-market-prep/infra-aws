###############################################################################
# external-dns layer - the AWS-side prerequisites for external-dns managing
# records in the platform hosted zone. State key shared/external-dns, separate
# from the eks, karpenter and cert-manager layers. The in-cluster install
# (HelmRelease, domain filter, txt-owner-id) lives in gitops-flux and consumes
# the role ARN output below.
#
...
# "seam without shared state" idea the karpenter and cert-manager layers use.
###############################################################################
...
terraform {
  ...
  backend "s3" {
    ...
    key          = "shared/external-dns/terraform.tfstate"
    region       = "us-east-1"
    encrypt      = true
...
locals {
  ...
  layer  = yamldecode(file("${path.module}/config.yaml")).external_dns
...
module "external_dns" {
  ...
  # Same boundary the eks, karpenter and cert-manager roles carry - a
  # PlatformEngineer may only create IAM roles that carry the engineer boundary.
